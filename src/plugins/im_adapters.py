...
class FeishuAdapter(IMAdapterBase):
    ...
    async def _process_message_async(self, message_id: str, user_id: str, content: str, chat_type: str, msg_type: str = '', attachments: List[Dict] = None):
        try:
            ...
            if file_key:
                logger.info(f"开始处理文件消息: file_key={file_key}, file_name={file_name}")
                file_content = await self._read_feishu_file(file_key, message_id)
                logger.info(f"文件读取结果: file_key={file_key}, content_length={len(file_content) if file_content else 0}")
                
                if file_content:
                    # 将文件内容存储到向量数据库
                    try:
                        doc_ids = rag_manager.add_large_document(
                            content=file_content,
                            metadata={
                                "user_id": user_id,
                                "file_key": file_key,
                        ...
                        logger.info(f"文件内容已存储到向量数据库: {file_name}, doc_ids={doc_ids}, chunk_count={len(doc_ids)}")
                        
                        # 将文件内容添加到消息文本中，便于后续处理（保留完整内容）
                        text = f"文件：{file_name}\n\n{file_content}"
                        logger.info(f"文件内容已添加到消息文本: {file_name}, content_length={len(file_content)}")
                    except Exception as e:
                        logger.error(f"存储文件内容失败: {str(e)}", exc_info=True)
                        text = f"文件：{file_name}\n\n（文件内容存储失败）"
                else:
                    # 文件读取失败，记录错误但继续处理消息
                    logger.error(f"文件读取失败: file_key={file_key}, file_name={file_name}")
                    text = f"文件：{file_name}\n\n（文件内容读取失败，请稍后重试）"
